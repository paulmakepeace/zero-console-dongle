...
| `/api/status`        | GET    | includes an `observed` object: what this bike actually does, next to what `config.h` assumes it does. Boot time from pin 8 rising, the longest answer seen, the delay from the last byte to the line going down, the hibernate interval it announces, and how many answers ended with the prompt the framer matches. A Zero firmware change shows up as one of these no longer matching its assumed pair; `prompts` reading zero on a bike that is awake means the prompt itself has changed and command framing is lost |
