...
// Bench knobs, not persisted: the grace before a sleep and the longest chunk. Boot restores the defaults.
void sleepSetTiming(uint32_t graceMs, uint32_t chunkS);
uint32_t sleepGraceMs();
uint32_t sleepChunkS();
